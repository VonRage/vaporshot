...
func initialize(root : Dictionary, author : String, index : int) -> void:
	...
	if item_dict.has("priority"):
		...
		_priority_label.text = str(item_dict.priority);
	...
	_stub_view.hint_tooltip = item_dict.text;
	_text_edit.text = item_dict.text;
	_text_edit.clear_undo_history();
	refresh_tags();
	minimize(!item_dict.text.empty());
...
func refresh_tags(update_edit : bool = true) -> void:
	...
	for tag in tags:
		...
		var tag_button : Label = TagButton.instance();
		tag_button.modulate = color;
		tag_button.text = tag;
		tag_button.connect("pressed", self, "_on_tag_button_pressed");
		_tag_hbox.add_child(tag_button);
	if update_edit:
		_tags_edit.text = PoolStringArray(tags).join(", ");
...
func update_priority_range(low : float, high : float) -> void:
	...
	_priority_label.modulate = priority_temp.interpolate(temp);


func _on_stub_button_pressed() -> void:
	if (Input.get_mouse_button_mask() & BUTTON_MASK_RIGHT) != 0:
		OS.clipboard = item_dict.text.split("\n")[0];
...
func _on_text_changed() -> void:
	...
	_stub_view.hint_tooltip = _text_edit.text;
...
func _on_priority_changed(value : float) -> void:
	if is_nan(value) || is_inf(value):
		...
		_priority_label.visible = true;
		_priority_label.text = str(value);
		_priority_clear.disabled = false;
