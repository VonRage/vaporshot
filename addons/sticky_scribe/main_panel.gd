...
export(String, FILE) var user_config := "user://settings.cfg";
export(String, FILE) var issues_json := "res://project.issues";
export(String, FILE) var git_index_lock_file := "res://.git/index.lock";
...
func _initialize() -> void:
	...
	_sort.connect("id_pressed", self, "_on_sort_method_pressed");
	_read_settings();
...
func _on_author_changed(name : String) -> void:
	_write_settings();
	if _author_edit.text.empty():
		_new_button.disabled = true;
		_new_button.hint_tooltip = "Author field required to open a new sticky";
	else:
		_new_button.disabled = false;
		_new_button.hint_tooltip = "";
...
func _notification(what):
	if _initialized && what == MainLoop.NOTIFICATION_WM_FOCUS_IN:
		_refresh();


func _refresh() -> void:
	var file := File.new();
	if !file.file_exists(issues_json):
		_clear_state();
		return;
	var modified = file.get_modified_time(issues_json);
	...
	_clear_state();
	for i in range(5):
		if !file.file_exists(git_index_lock_file):
			break;
		if i < 4:
			print("git index busy, trying again in a second, attempt #", i + 1);
			OS.delay_msec(500);
		else:
			printerr("git index still locked after a few attempts, tab out then back in once ongoing git operations have finished");
			return;
	var err := file.open(issues_json, File.READ);
	if err != OK:
		print("failed to open issues file ", issues_json, " : ", err);
		return;
	_modified_time = file.get_modified_time(issues_json); # get modified time again while open just in case
	var result := JSON.parse(file.get_as_text());
	file.close();
	if result.error != OK:
		printerr("failed to parse issues json ", issues_json, " : ", result.error_string, " @ ", result.error_line);
		return;
	_root_dictionary = result.result as Dictionary;
	...
	for author in _root_dictionary.issues.keys():
		...
		for i in range(author_array.size()):
			var issue_item := IssueItem.instance();
			_issue_vbox.add_child(issue_item);
			issue_item.initialize(_root_dictionary, author, i);
			issue_item.connect("changed", self, "_on_issue_item_changed");
			issue_item.connect("toggle_tag_color", self, "_on_toggle_tag_color");
			issue_item.connect("minimize_changed", self, "_on_minimize_item_changed");
	_write_lock = false;
...
func _save() -> void:
	...
	for author in dupe.issues.keys():
		...
		for i in range(author_array.size() - 1, -1, -1): # reverse-iterate
			...
			if item.has("deleted"):
				author_array.remove(i);
	
	var file := File.new();
	var out := JSON.print(dupe, "\t");
	if file.get_modified_time(issues_json) != _modified_time:
	...
	var err = file.open(issues_json, File.WRITE);
	if err != OK:
		printerr("failed to open issues file for write ", issues_json, " : ", err);
		return;
	...
	func sort(lhs : Node, rhs : Node) -> bool:
		...
		match option:
			SortOptions.LowPriorityFirst:
				reverse = true;
		match option: # continue feature will be removed in godot 4 so this is how we cope
...
func _add_palette_color(color : Color):
	if _tag_palette_grid.has_node(color.to_html(false)):
		return;
	var button : Button = ColorButton.instance();
	_tag_palette_grid.add_child(button);
	button.name = color.to_html(false);
	button.modulate = color;
	var height := button.get_font("font").get_height() + button.get_stylebox("normal").get_minimum_size().y;
	button.rect_min_size = Vector2(height, height);
	button.connect("pressed", self, "_on_set_tag_color", [color]);


func _on_toggle_tag_color(tag : String):
	_current_selected_tag = tag;
	_tag_palette_popup.popup();
	_tag_palette_popup.set_as_minsize();
	_tag_palette_popup.rect_position = get_global_mouse_position();
...
func _rebuild_filter_expression() -> void:
	var sub_expressions := PoolStringArray();
...
#			pass;
		...
		PriorityFilterMode.LessEqual:
			sub_expressions.append("priority <= " + str(_priority_edit.value));
	if !_search_edit.text.empty():
		sub_expressions.append("\"" + _search_edit.text.c_escape() + "\"" + " in text");
	_expression_edit.text = sub_expressions.join(" and ");
...
func _refilter_issues() -> void: # TBD: I can probably make this cleaner, am sleepy (-_-).zZ
	...
	for child in _issue_vbox.get_children():
		child.visible = true;
	if _expression_edit.text.empty():
