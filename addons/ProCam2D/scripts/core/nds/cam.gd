...
var smooth_offset: bool = true setget set_smooth_offset
...
var use_h_margins: bool = false setget set_use_h_margins
var use_v_margins: bool = false setget set_use_v_margins
...
func _gather_influence_nodes() -> void:
	...
	for node in _rooms + _paths:
		if not node.is_connected("priority_changed", self, "_on_node_priority_changed"):
			node.connect("priority_changed", self, "_on_node_priority_changed")
	for node in _rooms + _magnets + _zooms:
...
func _get_nodes_in_group(group_name: String) -> Array:
	...
	nodes.sort_custom(PCamUtils, "_sort_by_priority")
	return nodes
...
func _apply_cinematic_influences(pos: Vector2) -> Vector2:
	if not _playing_cinematic or _active_cinematics.empty():
		return pos
...
func _apply_addons_by_stage(stage: String, delta: float) -> void:
	...
	stage_addons.sort_custom(PCamUtils, "_sort_by_priority")
...
func _calculate_target_rotation() -> float:
	if _targets.empty() or _playing_cinematic:
...
func _calculate_target_zoom() -> float:
	if not auto_zoom or _targets.empty() or follow_mode == FollowMode.SINGLE_TARGET or _playing_cinematic:
		return zoom
...
func start_cinematic(cinematic_id) -> void:
	...
	if _active_cinematics.empty():
		print("No cinematics found with id: ", id_string)
		return
	_active_cinematics.sort_custom(PCamUtils, "_sort_by_priority")
